...
marketing_breakdown_initial <- ggplot(data = postworkshopsurveys_named,
       aes(x = reorder(ReducedCategories,ReducedCategories, length),
...
These words suggest that emailed campus newsletters (bulletin, insider, OSCER, Monday Memo), librarians, professors, and previous workshops were methods that reached these attendees.  Later data cleaning to incorporate emails/professors/word-of-mouth into the existing categories could be useful.
...
# reorder graph by top categories
...
Class requirements, direct emails from OU Libraries, personal recommendations from a librarian, and announcement in class are among top effective ways to get people to attend vs register.
...
```{r effectivenessGraph}
marketing_effectiveness <- all_marketing[all_marketing$ReducedCategories!="Walk-in",] %>%
  group_by(ReducedCategories,
           DataType) %>%
  summarize(count_per_each = n()) %>%
ggplot(
       aes(x = reorder(x=ReducedCategories,-count_per_each),
           y = count_per_each,
           fill = DataType)) +
...
This subset study was conducted by B. Narr and Claire Curry in 2023 and has not been re-run with new data, as it involved hand-collecting data.
