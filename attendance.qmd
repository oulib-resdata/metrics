...
For workshops (whether requested by patrons, or scheduled independently by librarians), total people reached per year reached a new high in 2025.

Workshops requested by someone either standalone (such as by a department or research group) or within a course have the highest attendances.  For workshops that we as librarians independently schedule,

  - for most topics, virtual and hybrid workshops have higher per-event attendance (6.4) than in-person (4.3) in the last two calendar years (2023-2025)

  - a few topics have higher per-event attendance (15) than others regardless of format

  - more detailed event descriptions seem to get higher per-event attendance than title only descriptions
...
The total number of people reached has no strong trends between 2021-2024 and has increased in 2025 to a new high.

```{r, total_reached_over_time, echo=FALSE, message = FALSE, warning = FALSE}

(attendance_table <- attendance_named %>%
...
attendance_table_long <- pivot_longer(data = attendance_table,
                                      cols = `2018`:`2025`,
                                      names_to = "year",
                                      values_to = "people_reached")

ggplot(data = attendance_table_long
       ) +
         geom_point(mapping = aes(x = as.numeric(year),
                     y = people_reached,
                     color = Is.Course,
                     linetype = Is.Course,
                     shape = Is.Course)) +
  geom_path(mapping = aes(x = as.numeric(year),
                     y = people_reached,
                     color = Is.Course,
                     linetype = Is.Course,
                     shape = Is.Course)) +
  geom_vline(xintercept = 2020,
             color = "gray") +
  # https://www.ou.edu/web/news_events/articles/news_2021/returning-to-in-person-work-on-the-norman-campus August 2021
  geom_vline(xintercept = 2021.67, color = "gray")+
  geom_text(mapping = aes(x = 2020.8, y = 650), label = "closure", color = "gray")+
  theme_bw() +
  theme_classic() +
  labs(x = "Calendar year", y = "Total people reached per year")
...
topics_common_ctree <- attendance_named %>%
...
- format
  ...
  - Remaining workshops have slightly higher (virtual and hybrid with an average of 6.4) or lower (4.3 in person) attendance based on format 
...
att_tree <- ctree(Total.Attendees ~ 
                    ...
                    as.factor(Semester),
                  data = attendance_named [
                    attendance_named$Is.Course == "Not a course" &
                      attendance_named$Requested. == "No"&
                      attendance_named$year <= this_year &
                      attendance_named$year >= last_two_years &
                      attendance_named$WorkshopCode %in% topics_common_ctree$WorkshopCode,],
                  na.action = na.omit
                  )
...
```{r attendance_over_time, echo=FALSE, message = FALSE, warning = FALSE}
 # plot_helpers <- attendance_named[attendance_named$Requested.=="Y",] %>%
