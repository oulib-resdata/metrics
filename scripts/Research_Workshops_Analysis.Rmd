...
For scheduled workshops with a known three week advertising period, none of the following variables impact attendance numbers in a conditional inference tree analysis.
...
When we include whether a workshop was in a course, requested or not, and had a known three-week advertising period, only whether a workshop was in a course was a deciding factor in increased attendance numbers.
...
postworkshopsurvey <- read.csv ("C:/Users/Brianna_Narr.ULHKH1JW3/Downloads/PostWorkshopSurveyFinal.csv", header=TRUE, stringsAsFactors=FALSE)
...
workshopliked <- readLines("C:/Users/Brianna_Narr.ULHKH1JW3/Documents/Documents_narr1655/WorkshopLiked.txt")
likedcorpus <- Corpus(VectorSource(workshopliked))

likedcorpus <- tm_map(likedcorpus, content_transformer(tolower))
likedcorpus <- tm_map(likedcorpus, removeNumbers)
likedcorpus <- tm_map(likedcorpus, removeWords, stopwords("english"))
likedcorpus <- tm_map(likedcorpus, removeWords, c("liked")) 
likedcorpus <- tm_map(likedcorpus, removePunctuation)
...
workshopimprove <- readLines("C:/Users/Brianna_Narr.ULHKH1JW3/Documents/Documents_narr1655/WorkshopNeedsImprovement.txt")
improvecorpus <- Corpus(VectorSource(workshopimprove))

improvecorpus <- tm_map(improvecorpus, content_transformer(tolower))
improvecorpus <- tm_map(improvecorpus, removeNumbers)
improvecorpus <- tm_map(improvecorpus, removeWords, stopwords("english"))
improvecorpus <- tm_map(improvecorpus, removeWords, c("think", "maybe")) 
improvecorpus <- tm_map(improvecorpus, removePunctuation)
...
v2 <- sort(rowSums(improvematrix), decreasing = TRUE)
improvedf <- data.frame(word = names(v2), freq=v2)
```
...
```{r workshop_feedback_othertopics_code, include=FALSE}
workshopcomments <- readLines("C:/Users/Brianna_Narr.ULHKH1JW3/Documents/Documents_narr1655/WorkshopOtherComments.txt")
...
commentscorpus <- tm_map(commentscorpus, removeWords, c("think", "maybe")) 
...
v4 <- sort(rowSums(commentsmatrix), decreasing = TRUE)
commentsdf <- data.frame(word = names(v4), freq=v4)
```
