...
```{r data_osf, include=FALSE, eval = FALSE}
# Load data from OSF
...
```{r data_import_registration_marketing, include=FALSE}
# Event registration 
...
```{r, data_postworkshop_import, include = FALSE}

# Post-workshop surveys from libwizard only
## Used for both feedback AND marketing

#same as file loaded below.  CMC TODO: merge with below where I pull in report.csv
...
postworkshopsurveys <- FeedbackActuallyAttended2 %>%
...
```{r data_marketing_clean_up, include=FALSE}
...
postworkshopsurveys$How.did.you.hear.about.this.workshop.[postworkshopsurveys$How.did.you.hear.about.this.workshop.=="Email from OU Libraries"] <- "Direct Email from OU Libraries (Source Unknown)"
postworkshopsurveys$How.did.you.hear.about.this.workshop.[postworkshopsurveys$How.did.you.hear.about.this.workshop.=="Colleagues"] <- "Colleagues (Forwarded email, word of mouth)"
postworkshopsurveys$How.did.you.hear.about.this.workshop.[postworkshopsurveys$How.did.you.hear.about.this.workshop.=="Supervisor"] <- "Supervisor (Forwarded email, word of mouth)"
postworkshopsurveys$How.did.you.hear.about.this.workshop.[postworkshopsurveys$How.did.you.hear.about.this.workshop.=="Flyer"] <- "Flyer (Paper or Digital Unspecified)"

postworkshopsurveys$ReducedCategories <- postworkshopsurveys$How.did.you.hear.about.this.workshop.
postworkshopsurveys$ReducedCategories[!(postworkshopsurveys$How.did.you.hear.about.this.workshop. %in% marketing_current_categories)] <- "Other (please describe)"
postworkshopsurveys$OtherDescriptions[!(postworkshopsurveys$How.did.you.hear.about.this.workshop. %in% marketing_current_categories)] <- postworkshopsurveys$How.did.you.hear.about.this.workshop.[!(postworkshopsurveys$How.did.you.hear.about.this.workshop. %in% marketing_current_categories)]
```
...
# Month, day, year, hour, minute, seconds
postworkshopsurveys$Date_2 <- mdy_hms(postworkshopsurveys$What.date.was.the.workshop.,
                                                           truncated = 3)


# separate year, month, date, times
postworkshopsurveys$year <- year(postworkshopsurveys$Date_2)
postworkshopsurveys$month <- month(postworkshopsurveys$Date_2)
postworkshopsurveys$day <- day(postworkshopsurveys$Date_2)
postworkshopsurveys$wday <- wday(postworkshopsurveys$Date_2,
                                      week_start = 1, #1 = Monday,
                                      label = TRUE    # days of weeks as characters
                                      )

# Overwrite original column with formatted date
postworkshopsurveys$Date <-
  format(postworkshopsurveys$Date_2, "%m/%d/%Y")
...
postworkshopsurveys$Valuable.for.Program <- factor(x = postworkshopsurveys$Valuable.for.Program,
                                                  levels = c("No answer",
                                                             "Not applicable", 
                                                             "Strongly agree",
                                                             "Agree",
...
Finally, the tidied feedback and attendance data are combined to compare feedback with per-workshop characteristics such as format of instruction.
```{r, workshop_feedback_format_data, echo = FALSE}
joined_post <- left_join(postworkshopsurveys, 
...
marketing_breakdown_initial <- ggplot(data = postworkshopsurveys,
...
other_source_Preregistered <- ggwordcloud_analysis_complete (df = postworkshopsurveys,
                                                             min_words = 2,
                                                             max_size = 70,
...
actually_attended_only <- data.frame("DataType" = "surveyed_attendees",
                              "ReducedCategories" = postworkshopsurveys[,"ReducedCategories"])
...
chi_newkno <- chisq.test(x = na.post$Location.of.Outreach,
           y = na.post$Using.New.Knowledge.,
...
# run test for significance
chi_career <- chisq.test(x = na.post$Location.of.Outreach,
           y = na.post$Valuable.for.Career,
           simulate.p.value = TRUE)
...
# run test for significance
chi_careerclass <- chisq.test(x = na.post$Classification,
           y = na.post$Valuable.for.Career,
           simulate.p.value = TRUE)
...
# run test for significance
chi_teach <- chisq.test(x = na.post$Location.of.Outreach,
           y = na.post$Valuable.for.Teaching,
           simulate.p.value = TRUE)
...
# run test for significance
chi_skills <- chisq.test(x = na.post$Location.of.Outreach,
           y = na.post$Rate.Skills,
           simulate.p.value = TRUE)
...
workshoptopics<- readLines("C:/Users/Brianna_Narr.ULHKH1JW3/Documents/Documents_narr1655/WorkshopOtherTopics_Cleaned.txt")
