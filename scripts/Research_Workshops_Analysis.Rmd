...
# heat  map as https://stackoverflow.com/questions/61676055/heatmap-data-visualization-with-categorical-count-and-time-in-r
joined_post %>%
  group_by(Location.of.Outreach,
           Valuable.for.Program) %>%
  summarize(count_per_feeling = n()) %>%
  ggplot(mapping = aes(x = Location.of.Outreach,
                                            y = Valuable.for.Program,
                       fill = count_per_feeling)) +
  geom_tile()
